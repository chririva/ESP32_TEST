...
#define DFL_SERIAL              "1"
#define DFL_SELFSIGN            1 //0
#define DFL_IS_CA               1 //0 (capable of signing other certificates)!!
...
void selfsigned_cert_write( void )
{
    ...
    /*for( i = 1; i < argc; i++ )
    {
        ...
        *q++ = '\0';
        ...
        {
            optt.authority_identifier = atoi( q );
            if( optt.authority_identifier != 0 &&
                optt.authority_identifier != 1 )
            {
        ...
        {
            optt.subject_identifier = atoi( q );
            if( optt.subject_identifier != 0 &&
                optt.subject_identifier != 1 )
            {
                mbedtls_printf( "Invalid argument for option %s\n", p );
                goto usage;
            }
        ...
        {
            optt.basic_constraints = atoi( q );
            if( optt.basic_constraints != 0 &&
                optt.basic_constraints != 1 )
            {
                mbedtls_printf( "Invalid argument for option %s\n", p );
                goto usage;
            }
        ...
        {
            if( strcmp( q, "SHA1" ) == 0 )
                optt.md = MBEDTLS_MD_SHA1;
            else if( strcmp( q, "SHA256" ) == 0 )
                optt.md = MBEDTLS_MD_SHA256;
            else if( strcmp( q, "SHA512" ) == 0 )
                optt.md = MBEDTLS_MD_SHA512;
            else if( strcmp( q, "MD2" ) == 0 )
                optt.md = MBEDTLS_MD_MD2;
            else if( strcmp( q, "MD4" ) == 0 )
                optt.md = MBEDTLS_MD_MD4;
            else if( strcmp( q, "MD5" ) == 0 )
                optt.md = MBEDTLS_MD_MD5;
            else
            {
                mbedtls_printf( "Invalid argument for option %s\n", p );
                goto usage;
        ...
        {
            optt.version = atoi( q );
            if( optt.version < 1 || optt.version > 3 )
            {
                mbedtls_printf( "Invalid argument for option %s\n", p );
                goto usage;
            }
        ...
        {
            optt.selfsign = atoi( q );
            if( optt.selfsign < 0 || optt.selfsign > 1 )
            {
                mbedtls_printf( "Invalid argument for option %s\n", p );
                goto usage;
            }
        ...
        {
            optt.max_pathlen = atoi( q );
            if( optt.max_pathlen < -1 || optt.max_pathlen > 127 )
            {
                mbedtls_printf( "Invalid argument for option %s\n", p );
                goto usage;
            }
        ...
        else if( strcmp( p, "key_usage" ) == 0 )
        {
            while( q != NULL )
            {
                ...
                if( strcmp( q, "digital_signature" ) == 0 )
                    optt.key_usage |= MBEDTLS_X509_KU_DIGITAL_SIGNATURE;
                else if( strcmp( q, "non_repudiation" ) == 0 )
                    optt.key_usage |= MBEDTLS_X509_KU_NON_REPUDIATION;
                else if( strcmp( q, "key_encipherment" ) == 0 )
                    optt.key_usage |= MBEDTLS_X509_KU_KEY_ENCIPHERMENT;
                else if( strcmp( q, "data_encipherment" ) == 0 )
                    optt.key_usage |= MBEDTLS_X509_KU_DATA_ENCIPHERMENT;
                else if( strcmp( q, "key_agreement" ) == 0 )
                    optt.key_usage |= MBEDTLS_X509_KU_KEY_AGREEMENT;
                else if( strcmp( q, "key_cert_sign" ) == 0 )
                    optt.key_usage |= MBEDTLS_X509_KU_KEY_CERT_SIGN;
                else if( strcmp( q, "crl_sign" ) == 0 )
                    optt.key_usage |= MBEDTLS_X509_KU_CRL_SIGN;
                else
                {
                    mbedtls_printf( "Invalid argument for option %s\n", p );
                    goto usage;
        ...
        else if( strcmp( p, "ns_cert_type" ) == 0 )
        {
            while( q != NULL )
            {
                ...
                if( strcmp( q, "ssl_client" ) == 0 )
                    optt.ns_cert_type |= MBEDTLS_X509_NS_CERT_TYPE_SSL_CLIENT;
                else if( strcmp( q, "ssl_server" ) == 0 )
                    optt.ns_cert_type |= MBEDTLS_X509_NS_CERT_TYPE_SSL_SERVER;
                else if( strcmp( q, "email" ) == 0 )
                    optt.ns_cert_type |= MBEDTLS_X509_NS_CERT_TYPE_EMAIL;
                else if( strcmp( q, "object_signing" ) == 0 )
                    optt.ns_cert_type |= MBEDTLS_X509_NS_CERT_TYPE_OBJECT_SIGNING;
                else if( strcmp( q, "ssl_ca" ) == 0 )
                    optt.ns_cert_type |= MBEDTLS_X509_NS_CERT_TYPE_SSL_CA;
                else if( strcmp( q, "email_ca" ) == 0 )
                    optt.ns_cert_type |= MBEDTLS_X509_NS_CERT_TYPE_EMAIL_CA;
                else if( strcmp( q, "object_signing_ca" ) == 0 )
                    optt.ns_cert_type |= MBEDTLS_X509_NS_CERT_TYPE_OBJECT_SIGNING_CA;
                else
                {
                    mbedtls_printf( "Invalid argument for option %s\n", p );
                    goto usage;
    ...
    //
    if( !optt.selfsign && strlen( optt.issuer_crt ) )
    {
        /*
        ...
        if( ( ret = mbedtls_x509_crt_parse_file( &issuer_crt, optt.issuer_crt ) ) != 0 )
        {
            mbedtls_strerror( ret, buf, 1024 );
    ...
    //
    if( !optt.selfsign && strlen( optt.request_file ) )
    {
        /*
         * 1.0.b. Load the CSR
         */
        ...
        if( ( ret = mbedtls_x509_csr_parse_file( &csr, optt.request_file ) ) != 0 )
        {
            mbedtls_strerror( ret, buf, 1024 );
        ...
        optt.subject_name = subject_name;
        subject_key = &csr.pk;
     ...
     * 1.1. Load the keys
     */
    if( !optt.selfsign && !strlen( optt.request_file ) ) //TODO: capire se mi serve, visto che è selfsign quindi subject_key=issuer_key ??
    {
        mbedtls_printf( "  . Loading the subject key ..." );
        fflush( stdout );

        ret = mbedtls_pk_parse_keyfile( &loaded_subject_key, optt.subject_key,
                                 optt.subject_pwd );
        if( ret != 0 )
        {
            mbedtls_strerror( ret, buf, 1024 );
    ...
    ret = mbedtls_pk_parse_keyfile( &loaded_issuer_key, optt.issuer_key,
                             optt.issuer_pwd );
    if( ret != 0 )
    {
        mbedtls_strerror( ret, buf, 1024 );
